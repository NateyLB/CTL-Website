...
const CartCard = (props) => {
  ...
  if (props.products.products.length > 0) {
    const index1 = props.products.products.findIndex(
      (element) => element.product_id === product.product_id
    );
    const index2 = props.products.products[index1].sizes.findIndex(
      (element) => element.id === product.size.id
    );
  ...
  return (
    <div className="cart-card">
      ...
      <span className="cancel" onClick={removeFromCart}>
        {window.screen.width >= 768 ? "REMOVE" : "X"}
      </span>
    </div>
  );
};
