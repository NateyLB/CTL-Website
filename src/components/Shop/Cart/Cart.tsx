...
const Cart = (props) => {
  ...
  const createCartCards = () => {
    return props.cart.cart.map((product, index) => (
      <CartCard
  ...
  return (
    <div className={props.checkout ? "cart-overlay" : "billing-confirm"}>
      <div>
        <img
          className="CTL-logo"
          src={require("../../../resources/favicon_io_transparent/android-chrome-512x512.png")}
          alt="Lost Cause"
        />
        {props.checkout ? (
      ...
      {props.checkout && totalPrice > 0 ? (
        props.cart.cart.length > 0 ? (
          <Link to="/checkout">
            <div className="checkout-button" onClick={closeCart}>
              Checkout
            </div>
          </Link>
        ) : null
