...
const Product = (props) => {
  ...
  const createSizeButtons = () => {
    ...
    return colorSizes.map((size, index) => {
      ...
      return (
        <div
          style={sizeButtonStyles[index]}
          className="product-size-button"
          key={size.size + size.color + size.quantity}
          onClick={() => {
            toggleStyle(sizeButtonStylesCopy, setSizeButtonStyles, index);
            setSizeToBuy(size);
  ...
  const createColorButtons = () => {
    ...
    return colors.map((color, index) => {
      ...
      return (
        <div
          style={colorButtonStyles[index]}
          className="product-size-button"
          key={color}
          onClick={() => {
            setColor(color);
            toggleStyle(colorButtonStylesCopy, setColorButtonStyles, index);
          }}
        >
          {color}
        </div>
  ...
  const addToCart = () => {
    const formQuantity = (document.getElementById(
      "product-quantity"
    ...
    if (localStorage.getItem("cart")) {
      ...
      for (let i = 0; i < cart.length; i++) {
        if (
          cart[i].product_id === productToBuy.product_id &&
          cart[i].size.id === productToBuy.size.id
        ) {
  ...
  return props.products.products.length ? (
    <div className="product-container">
      <Carousel
        key={match.params.index + product.product_id}
        img_urls={product.img_urls}
        edit={false}
      />
      <div className="product-info">
        ...
        <h2>Color</h2>
        <div className="product-button-container">{createColorButtons()}</div>
        {color !== "" ? <h2>Size</h2> : null}
        <div className="product-button-container">{createSizeButtons()}</div>
        {sizeToBuy === null ? null : (
