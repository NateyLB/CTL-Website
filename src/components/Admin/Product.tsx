  ...
  return edit === false ? (
    <div className="admin-product-card">
      <div className="admin-product-info">
        ...
        <div>
          <h2>Price:</h2> {props.product.price}
        </div>
        <div className="admin-product-stock">
          ...
          <div className="admin-product-stock-container">
            {props.product.sizes.map((size) => {
              return (
                <div
                  className="admin-product-stock-data-container"
                  key={size.size + size.color + size.quantity}
                >
                  <p>{size.size}</p> <p>{size.color}</p>{" "}
                  <p>Qty:{size.quantity}</p>
                </div>
              );
            })}
          </div>
        ...
        <div>
          <h2>Total in stock:</h2> {props.product.quantity}
        </div>
      </div>
      <Carousel
        key={`Carousel${props.product.product_id}`}
        img_urls={props.product.img_urls}
        edit={edit}
        index={props.index}
      />
      <i
        className="far fa-edit admin-product-edit-icon"
        onClick={toggleEdit}
      ></i>
    </div>
  ) : (
    <form className="admin-product-card">
      <div className="admin-product-info">
          ...
          <div className="admin-product-stock-container">
            {product.sizes.map((size, index) => {
              return editSizes[index] ? (
                <div
                  className="admin-product-stock-form-container"
                  key={size.size + size.color + size.quantity + "edit"}
                >
                  <input
                    type="text"
                    name="size"
                    value={sizeToEdit.size}
                  ...
                  <input
                    ...
                    step="5"
                    min="0"
                    value={sizeToEdit.quantity}
                    onChange={sizeHandler}
                    onClick={(event) => event.stopPropagation()}
                  />
                  <i
                    className="fas fa-times admin-size-edit-icon"
                    onClick={() => toggleEditSize(index)}
                  ></i>
                  <button
                    className="save-size"
                    onClick={(event) => saveSize(event, index)}
                  >
                    {" "}
                    Save{" "}
                  </button>
        ...
        <div>
          <h2>Total in stock:</h2> {product.quantity}
        </div>
        <button
          className="submit"
          onClick={(event) => updateProduct(event, props.index)}
        >
          {" "}
          Submit{" "}
        </button>
      ...
      />
      <i
        className="fas fa-times admin-product-edit-icon"
        onClick={toggleEdit}
      ></i>
    </form>
  );
...
export default connect(mapStateToProps, { updateProduct })(Product);
